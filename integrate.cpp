...
{   
    const int n_dim_min = 2; 
    const int n_dim_max = 15; 

    const long unsigned int integrations_pts = 1e7; 
    ...
    for (int dim = n_dim_min; dim<=n_dim_max; dim++) {
        ...
        auto volume = MontecarloIntegrate(integrations_pts, bounds, is_in_ball);
